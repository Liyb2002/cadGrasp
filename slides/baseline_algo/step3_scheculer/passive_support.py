...
sum(head Fy) - N = 0. All heads share N, so downward heads may cooperate with
...
def heads(wrenches, scale):
    ...
    return np.c_[raw*np.asarray(scale), raw[:, 1]]
...
def description():
    return dict(mode=MODE, enforced=True, inequality='sum(head_force_on_workpiece_y) >= 0',
