...
class PassiveSupportTests(unittest.TestCase):
    def test_hard_rest_pass_does_not_require_full_work_load_coverage(self):
        ...
        full=np.vstack([U.heads([np.asarray([0,0,1,0,0,0])],np.ones(6)),U.floor([],np.ones(6))])
        self.assertTrue(C.gravity_check(full,domain,np.ones(6))['passed'])
        mask=C.J.classify(full,np.array([np.asarray([0,0,1,0,0,0]),np.asarray([1,0,1,0,0,0])]))[0]
    ...
    def test_downward_head_cannot_borrow_the_object_floor_reaction(self):
        ...
        floor=np.array([np.asarray([0.,0.,1.,0.,1.,0.])])
        head=np.array([np.asarray([0.,0.,-1.,0.,-2.,0.])])
        target=np.array(np.asarray([0.,0.,1.,0.,0.,0.]))
    ...
    def test_downward_and_upward_heads_cooperate_across_groups(self):
        down=U.heads([np.asarray([1,0,-1,0,0,0])],self.scale)
        up=U.heads([np.asarray([0,0,1,0,0,0])],self.scale)
        full=C.I.merge_columns(self.slack,down,up)
        for magnitude in (.5,1.,2.):
            target=np.array(np.asarray([1.,0.,magnitude,0.,0.,0.]))
        ...
        report=C.verify_classification(full,np.array([np.asarray([1,0,1,0,0,0])]),np.array([True]))
    ...
    def test_real_b_pose2_downward_candidate_is_not_coverage(self):
        ...
        with selected_pose('pose_2'):
            ...
            gravity=np.array(np.asarray([0.,0.,1.,0.,0.,0.]))*problem.scale
