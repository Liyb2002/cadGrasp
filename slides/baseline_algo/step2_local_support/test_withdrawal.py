...
class WithdrawalTests(unittest.TestCase):
    def setUp(self):
        self.mesh=trimesh.creation.box(extents=[2., 2., 2.]);self.mesh.apply_translation([0, 0, 1.1])
    ...
    def test_continuous_ray_hits_remote_obstacle_despite_clear_start_and_end(self):
        head=trimesh.creation.box(extents=[.1, .1, .1]);head.apply_translation([-2., 0, 1.])
    ...
    def test_face_contact_allows_outward_and_tangent_but_not_inward(self):
        head=trimesh.creation.box(extents=[.1, .2, .2]);head.apply_translation([1.05, 0, 1.])
        self.assertTrue(self.analyzer.test([head],[1.,0,0])['clear'])
        self.assertTrue(self.analyzer.test([head],[0,0,1.])['clear'])
        self.assertFalse(self.analyzer.test([head],[-1.,0,0])['clear'])
        self.assertFalse(self.analyzer.test([head],[0,0,-1.])['clear'])
    ...
    def test_work_side_and_floor_are_locked_before_selecting_any_head(self):
        ...
        self.assertTrue(np.all(alive[:,0]<=W.NORMAL_TOL));self.assertTrue(np.all(alive[:,2]>=-W.NORMAL_TOL))
        self.assertTrue(np.any(alive[:,2]>.1));self.assertTrue(len(cat['work_face_locked_ids'])>0)

    def test_downward_facing_head_can_withdraw_horizontally_above_floor(self):
        head=trimesh.creation.box(extents=[.2, .2, .05]);head.apply_translation([0, 0, .075])
        self.assertTrue(self.analyzer.test([head],[1.,0,0])['clear'])
        self.assertFalse(self.analyzer.test([head],[0,0,1.])['clear'])

    def test_all_cells_must_share_the_same_ray(self):
        a=trimesh.creation.box(extents=[.1, .2, .2]);a.apply_translation([1.05, 0, 1.])
