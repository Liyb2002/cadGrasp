...
class BeltTests(unittest.TestCase):
    ...
    def test_surface_belt_retains_heads_and_avoids_work_faces(self):
        work = np.flatnonzero(self.mesh.face_normals[:, 1] > .9)
    ...
    def test_real_detached_thin_material_is_not_removed(self):
        ...
        thin = trimesh.creation.box([.1, 1e-6, .1]); thin.apply_translation([2., 0., 0.])
    ...
    def test_open_ring_retains_required_floor_hull(self):
        ...
        self.assertLess(base['cut_offset_m'], np.max(np.asarray(base['outer_xz_m'])[:, 0]))
    ...
    def test_clear_translation_is_found_and_continuously_replayed(self):
        piece = trimesh.creation.box([.1, .1, .1]); piece.apply_translation([-.6, 1., 0])
    ...
    def test_rotational_midpath_collision_is_rejected(self):
        obstacle = trimesh.creation.box([.2, .2, .2]); obstacle.apply_translation([0., .5, 1.])
        scene = B.Scene(obstacle)
        piece = trimesh.creation.box([.1, .1, .1]); piece.apply_translation([1., .5, 0.])
        origin = np.array([0., .5, 0.]); motion = np.array([0., 0., 0., 0., -np.pi, 0.])
    ...
    def test_clear_rotation_is_checked_as_a_continuous_arc(self):
        obstacle = trimesh.creation.box([.2, .2, .2]); obstacle.apply_translation([0., .5, 0.])
        scene = B.Scene(obstacle)
        piece = trimesh.creation.box([.1, .1, .1]); piece.apply_translation([1., .5, 0.])
        budget = dict(remaining=100, checked=0)
        self.assertTrue(P.segment(scene, [piece], np.array([0., .5, 0.]),
            np.array([0., 0., 0., 0., -np.pi, 0.]), 0., 1., budget))
        self.assertGreater(budget['checked'], 1)


    def test_rotation_padding_preserves_tangent_contact_plane(self):
        piece=trimesh.creation.box([.1, .1, .1]);piece.apply_translation([.55, 1, 0])
        budget=dict(remaining=200,checked=0)
        self.assertTrue(P.segment(self.scene,[piece],np.array([0.,1.,0.]),
            np.array([0.,0.,0.,-np.pi/3,0.,0.]),0.,1.,budget))

    def test_yaw_at_floor_does_not_invent_downward_motion(self):
        piece=trimesh.creation.box([.1, .1, .1]);piece.apply_translation([1., .05, 0])
        budget=dict(remaining=200,checked=0)
        self.assertTrue(P.segment(self.scene,[piece],np.zeros(3),
            np.array([0.,0.,0.,0.,-.5,0.]),0.,1.,budget))
    ...
    def test_analytic_floor_minimum_contains_intermediate_rotated_vertices(self):
        ...
        origin=np.array([.1,.2,.3]);motion=np.array(COORD.wrench([.1,.2,-.15,.8,-.3,.6]))
        _,low=P.rotation_enclosure(points,origin,motion,1.,.1,.8)
        sampled=min(P.transform(points,origin,1.,motion,t)[:,1].min() for t in np.linspace(.1,.8,1001))
    ...
    def test_piecewise_detour_replays_and_direct_shortcut_collides(self):
        ...
        piece=trimesh.creation.box([.1, .1, .1]);piece.apply_translation([-.7, 1., 0.])
        poses=[]
        for shift in ([0,0,0],[0,0,1],[1.4,0,1],[1.4,0,0]):
        ...
        self.assertFalse(PP.edge(self.scene,[piece],np.array([0.,1.,0.]),poses[0],poses[-1])[0])
    ...
    def test_original_object_floor_point_has_the_same_friction_cone(self):
        ...
        np.testing.assert_array_equal(normals[pivot],[[4,1,0],[-4,1,0],[0,1,4],[0,1,-4]])
        self.assertTrue(np.all(np.linalg.norm(COORD.floor(normals[pivot]),axis=1)<=4*normals[pivot,1]))
    ...
    def test_current_shared_body_reactions_replay_with_friction_at_both_feet(self):
        ...
        report,arrays=BA.bearing(domain,heads,floor,dict(pads_xz_m=[[[-1,-1],[1,-1],[1,1],[-1,1]]]))
