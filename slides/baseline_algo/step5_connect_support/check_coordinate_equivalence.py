"""Compare the rebuilt Y-up B design with its transported pre-migration reference.

The stored reference already uses Y-up. Check scientific loads and physical
...
def run(pose):
    ...
    assert str(ref['coordinate_system']) == 'y_up_xz_floor'
    ...
    report = dict(
        object='B', pose=pose, coordinate_system='y_up_xz_floor', complete=True, passed=True,
        ...
        original_z_up_reference_sha256=str(ref['original_z_up_reference_sha256']),
